  ...
  <div>
    <div class="avatar cursor-pointer" @mouseover="isHover=true" @mouseout="isHover=false">
      <div class="w-60 h-100 rounded-2xl relative">
        <img class="transition-transform duration-300" :src="character.background_image" :class="{'scale-110': isHover}" alt="">
        ...
        <div class="absolute left-4 top-54 avatar">
          <div class="w-16 rounded-full ring-white">
            <img :src="character.photo" alt="">
          </div>
        </div>
        <div class="absolute left-24 right-4 top-58 text-white font-bond line-clamp-1 break-all">
          {{ character.name }}
        </div>
